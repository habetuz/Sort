SHEETS_TRACKING.sheetsURL =
    "https://script.google.com/macros/s/AKfycbyCUxU91LZl7Chs0017YWMvYvEUzoOBcVxznNIRGu_D8F1K2Nhd5fLFi1V5xUQN8P1R/exec";
...
document
    .getElementById("button-subtractone")
    .addEventListener("click", function () {
        itemCount--;
        run();
    });
document
    .getElementById("bubble-sort-switch")
    .addEventListener("input", function () {
        toggleAlgorithm(bubbleSortAlgorithm, bubbleSort);
    });
document
    .getElementById("insert-sort-switch")
    .addEventListener("input", function () {
        toggleAlgorithm(insertSortAlgorithm, insertSort);
    });
document
    .getElementById("merge-sort-switch")
    .addEventListener("input", function () {
        toggleAlgorithm(mergeSortAlgorithm, mergeSort);
    });
document
    .getElementById("quick-sort-switch")
    .addEventListener("input", function () {
...
function bubbleSortAlgorithm(values) {
    document.getElementById("bubble-sort").style.height =
        (itemCount + 1) * 40 + 30 + "px";
...
function bubbleSortDrawMarker(i) {
    ...
    let start = SVG(
        document.getElementById(
            bubbleSort.idPrefix + (bubbleSort.step - 1) + "-" + startID
        )
    );
    let end = SVG(
        document.getElementById(
            bubbleSort.idPrefix + (bubbleSort.step - 1) + "-" + endID
        )
    );
    bubbleSort.svg
        .line(start.cx(), start.cy(), end.cx(), end.cy())
        .stroke({ color: "#546653", width: 50, linecap: "round" })
        .back();
}
...
function insertSortAlgorithm(values) {
    document.getElementById("insert-sort").style.height =
        (itemCount + 1) * 40 + 30 + "px";
...
function insertSortDrawMarker(i) {
    ...
    let start = SVG(
        document.getElementById(
            insertSort.idPrefix + (insertSort.step - 1) + "-" + startID
        )
    );
    let end = SVG(
        document.getElementById(
            insertSort.idPrefix + (insertSort.step - 1) + "-" + endID
        )
    );
    insertSort.svg
        .line(start.cx(), start.cy(), end.cx(), end.cy())
...
function mergeSortAlgorithm(values) {
    document.getElementById("merge-sort").style.height =
        (itemCount + 1) * 40 + 30 + "px";
    mergeSort.svg.clear();
    mergeSort.step = 0;
    mergeSort.items = [];
...
function mergeSortDrawMarker(i, j, a) {
    if (a < itemCount) {
        ...
        let unsortedEnd = SVG(
            document.getElementById(
                mergeSort.idPrefix + (mergeSort.step - 1) + "-" + unsortedEndID
            )
        );
        mergeSort.svg
            .line(
            ...
            .stroke({ color: "#4d4d4d", width: 50, linecap: "round" })
            .back();
    }
    ...
    let sortedStart = SVG(
        document.getElementById(
            mergeSort.idPrefix + (mergeSort.step - 1) + "-" + sortedStartID
        )
    );
    let sortedEnd = SVG(
        document.getElementById(
            mergeSort.idPrefix + (mergeSort.step - 1) + "-" + sortedEndID
        )
    );
    mergeSort.svg
        .line(
...
function quickSortAlgorithm(values) {
    document.getElementById("quick-sort").style.height =
        (itemCount + 1) * 40 + 30 + "px";
    quickSort.svg.clear();
    quickSort.step = 0;
    quickSort.items = [];
    ...
    let start = SVG(
        document.getElementById(
            quickSort.idPrefix + (quickSort.step - 1) + "-" + 0
        )
    );
    let end = SVG(
        document.getElementById(
            quickSort.idPrefix + (quickSort.step - 1) + "-" + (itemCount - 1)
        )
    );
    quickSort.svg
        .line(start.cx(), start.cy(), end.cx(), end.cy())
...
function quickSortDrawMarker(element, offset, finished) {
    ...
    let pivot = SVG(
        document.getElementById(
            quickSort.idPrefix + (quickSort.step - 2) + "-" + pivotID
        )
    );
    quickSort.svg
        .circle(50)
    ...
    let start = SVG(
        document.getElementById(
            quickSort.idPrefix + (quickSort.step - 2) + "-" + element.start
        )
    );
    let end = SVG(
        document.getElementById(
            quickSort.idPrefix + (quickSort.step - 2) + "-" + element.end
        )
    );
    quickSort.svg
        .line(start.cx(), start.cy(), end.cx(), end.cy())
    ...
    finished.forEach((item) => {
        let finishedStartID =
            (item - 1 >= 0 && item == element.end + 1) ||
            finished.includes(item - 1)
                ? item - 1
                : item;
        let finishedEndID =
            (item + 1 < itemCount && item == element.start - 1) ||
            finished.includes(item + 1)
                ? item + 1
                : item;
        //console.log("start id: " + finishedStartID + " | end id: " + finishedEndID + " | item: " + item)
        let finishedStart = SVG(
            document.getElementById(
                quickSort.idPrefix +
                    (quickSort.step - 2) +
                    "-" +
        ...
        let finishedEnd = SVG(
            document.getElementById(
                quickSort.idPrefix + (quickSort.step - 2) + "-" + finishedEndID
            )
        );
        quickSort.svg
            .line(
...
function publish(array, target) {
    ...
    for (let i = 0; i < array.length; i++) {
        ...
        if (element.gotCompared) {
            SVG(
                document.getElementById(
                    target.idPrefix + (target.step - 1) + "-" + i
                )
            ).stroke({ color: "#E8911A", width: 4 });
        }
                ...
                .stroke(
                    {
                        color: "#F5F6F7",
                        width: 2,
                    } /*mapColor(element.value, getMax(array))*/
                )
                .id(
                    target.idPrefix +
                        (target.step - 1) +
